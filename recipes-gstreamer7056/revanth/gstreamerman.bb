DESCRIPTION = "This is the mock implementation for GStreamer app."

LICENSE = "Apache-2.0"
LIC_FILES_CHKSUM = "file://${MANIFEST_PATH_META_RDK}/licenses/Apache-2.0;md5=3b83ef96387f14655fc854ddc3c6bd57"


PACKAGE_ARCH = "${MIDDLEWARE_ARCH}"


SRC_URI = "file://gstreamer"

DEPENDS += "gstreamer1.0 pkgconfig-native"


S = "${WORKDIR}/gstreamer"

do_compile() {
	oe_runmake
}

do_install() {
	install -d ${D}${bindir}
	install -m 0755 ${S}/output7056 ${D}${bindir}
}
#including files in the main sysroot
FILES:${PN} += "${bindir}/output7056"
