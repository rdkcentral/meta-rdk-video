...
# nmcli comes from NetworkManager
RDEPENDS:${PN}:append:ipstb-brcm974116sff = " networkmanager"
...
do_install:append:ipstb-brcm974116sff () {
    # Helper script
    install -m 0755 ${S}/wl02-p2p.sh ${D}${bindir}/wl02-p2p.sh

    # Systemd drop-in for wlan-p2p.service
...
# Package the Broadcom-only files
FILES:${PN}:append = " \
    ${bindir}/wl02-p2p.sh \
