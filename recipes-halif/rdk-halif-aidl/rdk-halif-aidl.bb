...
# vendor and mw carry same-named .so files at different partition paths;
# suppress mw libs from the global shlib DB so the scanner finds only one provider.
PRIVATE_LIBS = "lib*-v*-cpp.so"
