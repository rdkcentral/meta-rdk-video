...
EXTRA_OECMAKE:append:vdevice_x86-64-mw = " \
    ...
    -DCMAKE_SHARED_LINKER_FLAGS:STRING='${LDFLAGS} -Wl,-rpath-link,${STAGING_DIR_HOST}${prefix}/mw/lib/binder -Wl,--no-as-needed ${STAGING_LIBDIR}/mw/rdk-halif-aidl/libdeepsleep-v0.1.0.0-cpp.so ${STAGING_LIBDIR}/mw/rdk-halif-aidl/libbootreason-v0.1.0.0-cpp.so -Wl,--as-needed' \
"
...
do_configure:append:vdevice_x86-64-mw() {
    ...
    for library in libbinder.so libutils.so; do
        if [ ! -e "${STAGING_DIR_HOST}${prefix}/mw/lib/binder/${library}" ]; then
            bbfatal "Unable to locate staged ${library} under ${STAGING_DIR_HOST}${prefix}/mw/lib/binder"
        fi
    done
