...
arm_filtered_trace() {
# Re-resolve at boot/runtime because some images expose tracing paths early
    # but do not have tracefs/debugfs mounted yet.
    TRACEFS="$(resolve_tracefs || true)"
    if [ -z "$TRACEFS" ] || [ ! -f "$TRACEFS/tracing_on" ]; then
        log "ERROR: tracefs not ready (tracing_on missing)."
        log "       Checked: /sys/kernel/tracing and /sys/kernel/debug/tracing"
        return 1
    fi

    echo 0 > "$TRACEFS/tracing_on" 2>/dev/null || {
        log "ERROR: cannot write $TRACEFS/tracing_on"
        return 1
    }
    echo nop > "$TRACEFS/current_tracer" 2>/dev/null || {
        log "ERROR: cannot write $TRACEFS/current_tracer"
        return 1
    }
    : > "$TRACEFS/trace" 2>/dev/null || {
        log "ERROR: cannot clear $TRACEFS/trace"
        return 1
    }
    : > "$TRACEFS/set_event" 2>/dev/null || {
        log "ERROR: cannot clear $TRACEFS/set_event"
        return 1
    }
    echo 4096 > "$TRACEFS/buffer_size_kb" 2>/dev/null || {
        log "WARN: cannot set $TRACEFS/buffer_size_kb"
    }
    ...
    if [ -n "${TARGET_TID:-}" ]; then
        ...
        echo 1 > "$TRACEFS/tracing_on" 2>/dev/null || {
            log "ERROR: cannot enable tracing at $TRACEFS/tracing_on"
            return 1
        }
