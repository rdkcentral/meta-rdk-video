...
SYSLOG-NG_FILTER = " systemd dropbear gstreamer-cleanup rfc-config update-device-details applications vitalprocess-info iptables mount_log swupdate reboot-reason messages rdnssd ${@bb.utils.contains('DISTRO_FEATURES', 'ENABLE_NETWORKMANAGER', '', 'dibbler', d)}"
SYSLOG-NG_FILTER:append:rdkstb = "moca"
SYSLOG-NG_SERVICE_moca = "moca.service"
SYSLOG-NG_DESTINATION_moca = "mocaService.log"
SYSLOG-NG_LOGRATE_moca = "high"
...
SYSLOG-NG_SERVICE_dropbear:append = " checkforinterface.service "
SYSLOG-NG_DESTINATION_dropbear = "dropbear.log"
SYSLOG-NG_LOGRATE_dropbear = "medium"
SYSLOG-NG_SERVICE_checkforinterface = "checkforinterface.service"
SYSLOG-NG_DESTINATION_checkforinterface = "dropbear.log"
SYSLOG-NG_LOGRATE_checkforinterface = "medium"
...
SYSLOG-NG_SERVICE_dibbler = "${@bb.utils.contains('DISTRO_FEATURES', 'ENABLE_NETWORKMANAGER', '', 'dibbler.service', d)}"
SYSLOG-NG_DESTINATION_dibbler = "${@bb.utils.contains('DISTRO_FEATURES', 'ENABLE_NETWORKMANAGER', '', 'dibbler.log', d)}"
...
do_install() {
	...
	install -m 0644 ${S}/systemd_units/dropbear.service ${D}${systemd_unitdir}/system
        install -m 0644 ${S}/systemd_units/checkforinterface.service ${D}${systemd_unitdir}/system
...
do_install:append:rdkstb() {
        install -m 0644 ${S}/systemd_units/moca.service ${D}${systemd_unitdir}/system/moca.service
...
SYSTEMD_SERVICE:${PN}:append:rdkstb = " moca.service "
...
SYSTEMD_SERVICE:${PN} += "dibbler.service"
...
SYSTEMD_SERVICE:${PN} += "${@bb.utils.contains('DISTRO_FEATURES', 'ENABLE_NETWORKMANAGER', '', 'dnsmerge-udhcpc.path', d)}"
SYSTEMD_SERVICE:${PN} += "${@bb.utils.contains('DISTRO_FEATURES', 'ENABLE_NETWORKMANAGER', '', 'dnsmerge-upnp.path', d)}"
SYSTEMD_SERVICE:${PN} += "${@bb.utils.contains('DISTRO_FEATURES', 'ENABLE_NETWORKMANAGER', '', 'dnsmerge@.service', d)}"
SYSTEMD_SERVICE:${PN} += "${@bb.utils.contains('DISTRO_FEATURES', 'ENABLE_NETWORKMANAGER', '', 'dnsmerge-dibbler.path', d)}"
SYSTEMD_SERVICE:${PN} += "checkforinterface.service"
