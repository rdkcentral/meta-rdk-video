...
SRCREV = "1a08be8dac7470d4d8850ed4d80da392c93482e3"
...
ARTIFACT_NAME = "PLAYER_INTERFACE_${PLAYER_INTERFACE_ARTIFACTS_VERSION}.tgz"
...
do_create_artifacts() {
    ...
    echo "PLAYER_INTERFACE_BRANCH=${PLAYER_INTERFACE_RELEASE_TAG_NAME}" >> ${ARTIFACT_INFO_FILE}

    # Get the actual Git commit hash instead of AUTOREV
    if [ -d "${S}/.git" ]; then
        ACTUAL_REVISION=$(cd ${S} && git rev-parse HEAD)
        echo "PLAYER_INTERFACE_SRC_REV=$ACTUAL_REVISION" >> ${ARTIFACT_INFO_FILE}
    else
        echo "PLAYER_INTERFACE_SRC_REV=${SRCREV_player-interface} (from recipe)" >> ${ARTIFACT_INFO_FILE}
    fi
    ...
    mkdir -p ${DEPLOY_DIR_IMAGE}/PLAYER_INTERFACE_artifacts
    cp ${ARTIFACT_DIR}/${ARTIFACT_NAME} ${DEPLOY_DIR_IMAGE}/PLAYER_INTERFACE_artifacts/
}

do_deploy_artifacts() {
    if [ -f ${ARTIFACT_DIR}/${ARTIFACT_NAME} ]; then
        mkdir -p ${DEPLOY_DIR_WGT}/PLAYER_INTERFACE_artifacts
        cp -v ${ARTIFACT_DIR}/${ARTIFACT_NAME} ${DEPLOY_DIR_WGT}/PLAYER_INTERFACE_artifacts/
        echo "Copied ${ARTIFACT_DIR}/${ARTIFACT_NAME} to ${DEPLOY_DIR_WGT}/PLAYER_INTERFACE_artifacts"
    else
