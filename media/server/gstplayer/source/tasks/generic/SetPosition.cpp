...
void SetPosition::execute() const
{
    ...
    // Pre-seed lastKnownPosition with seek target. This covers the startup
    // window where lastKnownPosition might be uninitialised (-1) or stale (12ms).
    // This ensures the fallback in getPosition() returns the seek target
    // immediately, preventing the UI from freezing at the old position.
    m_context.lastKnownPosition = m_position;

    // Set atomic flags. These will be immediately visible to the position timer thread.
    m_context.seekInProgress = true;
    m_context.pendingSeekPosition = m_position;
    RIALTO_SERVER_LOG_ERROR("[Shibu]Seek in progress set, target: %" GST_TIME_FORMAT, GST_TIME_ARGS(m_position));
